...
def retime(task,adapter,basis,coeff,grid_n=801):
    t=perf_counter();path=SplinePath(basis,coeff);grid=np.unique(np.r_[np.linspace(0,1,grid_n),task.knots,basis.t])
...
def optimize(task,adapter,basis,initial,mode='smooth',timing=None,budget=30.,collision=None):
    ...
    grid=np.unique(np.r_[np.linspace(0,1,801),task.knots,basis.t])
