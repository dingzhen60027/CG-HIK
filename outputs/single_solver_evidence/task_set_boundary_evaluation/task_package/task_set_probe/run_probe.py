...
OUT=BASE/'measured';OUT.mkdir(exist_ok=False)
...
config=dict(seed=202609140713,anchors_per_family=30,families=['local','near_singular','near_limit','high_utilization'],alpha=[0.,.5,.95],dt=.02,position_tolerance=float(EPS[0]),orientation_tolerance=float(EPS[3]),kappa=1.,methods=['gn','tight','relative'],repeats=1,source='user supplied task_balance_completion_package.zip, numerical files unmodified',scope='one local Panda development coverage probe; not original-server verification, no input screening by solver results',offset='fixed independent unit translation and rotation directions per anchor; same direction for all alpha levels',accelerator=accelerator)
...
for family in config['families']:
 for j in range(config['anchors_per_family']):
  ...
  scale=rng.uniform(.75,.98) if family=='high_utilization' else rng.uniform(.1,.6)
  direction=rng.uniform(-1,1,7)
  if family=='high_utilization':direction=rng.choice([-1.,1.],7)*rng.uniform(.75,1.,7)
