...
from .elastic_study import configurations,verify_seal,ELASTIC,FOCUS_UID
...
def aggregate():
    ...
    for robot in cfg['robots']:
        folder=root/robot
        manifest=json.loads((folder/'completed.json').read_text());assert manifest['runs']==1000 and manifest['frames']==150000
        ...
        for index,s in enumerate(sorted(jobs,key=lambda s:s['run_id'])):
            ...
            for t,r in enumerate(rows):
                ...
                if method in ELASTIC:
                    ...
                    if r['direct_return']:
                        assert np.array_equal(q,r['backup_q']) and r['conic_calls']==0 and initial['met']
            ...
            if s['uid']==FOCUS_UID:focus.append(s)
            ...
            if (index+1)%100==0:print(f'read-only nonlinear audit {robot} {index+1}/1000',flush=True)
    ...
    for r in main:
        group=[s for s in summaries if s['robot']==r['robot'] and s['method']==r['method']];e=extras[(r['robot'],r['method'])]
        ...
        r.update(initial_gap_mean=avg(e['xi_before']),final_gap_mean=avg(e['xi_after']),normalized_gap_reduction_mean=avg(e['normalized_gap_reduction']))
    metrics=['completion','deadline_completion','total_latency_ns','frame_success','acceleration_rms','intervention_normalized_mean',
        'optimization_call_rate','partial_correction_rate','effective_command_adjustment_rate','normalized_shortfall_reduction_mean']
    units=[];pairs=[];changes=[]
    for robot in cfg['robots']:
        ...
        for method in METHODS_NEW:
            for baseline in [b for b in cfg['methods'] if b!=method and (b not in ELASTIC or b=='cr_ik_elastic_mu0')]:
                for metric in metrics:
                    a=[unit[(method,u)][metric] for u in uids];b=[unit[(baseline,u)][metric] for u in uids]
                    if any(x is None for x in a+b):continue
                    pairs.append(dict(robot=robot,method=method,baseline=baseline,metric=metric,trajectories=40,
    ...
    tables=dict(main_table=main,family_table=family,trajectory_units=units,paired_comparisons=pairs,gained_lost_uids=changes,
        first_failure_inputs=failures,focus_trajectory_15=focus,run_summaries=summaries,conditional_next_frame=conditional_rows,
    ...
    old.write_json(out/'manifest.json',dict(utc=old.utc(),sources=sources,code_sha256=old.sha(__file__),
        scope='observed development, not fresh test',independent_unit='40 trajectory UIDs per robot; 3 searches nested, Pink 1',
        ...
        exclusions=0,frame_calls=300000,runs=2000,selected_mu=None))
