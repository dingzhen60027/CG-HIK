"""Fair zero-reserve-cost baseline; the frozen primary runtime is untouched.

For a legal initial pair the nonnegative mu=0 objective is exactly zero.
Skipping its impossible improvement changes neither q/z nor causal state.
The solve body deliberately mirrors the frozen runtime, with only that shortcut
and its truthful method/decision labels changed. A source-equivalence test guards
the geometric recovery and all timing/feedback operations.
"""
...
from .elastic_runtime import ElasticInterventionIK, improves, OBJECTIVE_ATOL, OBJECTIVE_RTOL


class MuZeroAnalyticIK(ElasticInterventionIK):
    def __init__(self,*args,**kwargs):
        super().__init__(*args,mu=0.,**kwargs)
    ...
    def solve(self,position,rotation,previous,dt=.02):
        ...
        out.update(method='cr_ik_mu0_analytic',mu=self.mu,internal_ok=bool(selected or backup['internal_ok']),
