...
class CurrentOptimizer:
    ...
    def __init__(self, kin, verifier, urdf, method):
        ...
        self.settings = dict(maxiter=100, deadline_ms=20., single_start='actual previous_q',
            ...
            constraint_interior_guard=1e-7 if method == 'direct_sqp' else 0.,
    ...
    def solve(self, position, rotation, previous, dt=.02):
        ...
        def constraints(y):
            e, J = model(y)
            # A declared numerical interior guard exceeds SLSQP's feasibility
            # stopping error. Public verifier/tolerance remains unchanged.
            return np.array([1-e[:3]@e[:3], 1-e[3:]@e[3:]])-1e-7
