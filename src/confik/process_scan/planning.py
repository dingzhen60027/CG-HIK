...
def initialize(task,adapter,start_q,end_q,method,nodes=401,reference_pose=None):
    t=perf_counter();grid=np.unique(np.r_[np.linspace(0,1,nodes),task.knots]);poses=task.poses(grid) if reference_pose is None else reference_pose(grid)
...
def retime(task,adapter,basis,coeff,grid_n=801,speed_reserve=.98):
    ...
    dilation=max(1.,dense['velocity_ratio_max'],np.sqrt(dense['acceleration_ratio_max']))/speed_reserve
    result.update(x=result['x']/dilation**2,u=result['u']/dilation**2,
                  t=result['t']*dilation,duration=result['duration']*dilation,
                  validation_dilation=dilation,pre_dilation_validation=dense,speed_reserve=speed_reserve)
...
def joint_time_calibrated(task,adapter,basis,initial,timing,collision,budget=30.,
                          speed_reserve=.90,grid_n=201,derivative_check=False):
    """Same full-variable path/time NLP, nondimensionalized for Phase 1.5.

    Eliminate u exactly via x[k+1]-x[k]=2 ds u[k], and fixed endpoint x=0.
    All spline coefficients remain variables; no block selection or surrogate
    path is introduced. Final dense checks and common TOPPRA remain mandatory.
    """
    ...
    return dict(status=solver.stats()['return_status'],iterations=solver.stats().get('iter_count'),
        ...
        objective=float(ff)*timing['duration'],numeric_grid=grid,all_coefficients_variable=True,
